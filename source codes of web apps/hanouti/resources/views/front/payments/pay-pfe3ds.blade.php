<tr>
    ...
    <td>
        <form action="{{ route('checkout.store') }}" method="post" class="pull-right" id="payPalForm">
            {{ csrf_field() }}
            <input type="hidden" name="payment" value="{{ config('pay-pfe3ds.name') }}">
            ...
            <button onclick="return confirm('Are you sure?')" type="submit" class="btn btn-info pull-right">Pay with {{ ucwords($payment['name']) }} <i class="fa fa-umbrella"></i></button>
        </form>
    </td>
</tr>
